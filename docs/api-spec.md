...
- 일반 목록형 응답은 `page`, `page_size`, `total_count`를 `meta`에 포함합니다. 엔드포인트별 메타가 다르면 해당 섹션의 예시를 우선합니다.
...
|--------|------|------|------|
| `GET` | `/health` | 공통 | 구현됨 |
| `GET` | `/api/v1/buildings/nearby` | MVP | 구현됨 |
| `GET` | `/api/v1/buildings/{building_id}` | Phase 2 | 구현됨 |
| `GET` | `/api/v1/buildings/{building_id}/trades` | MVP | 구현됨 |
...
| `GET` | `/api/v1/livability/infra/nearby` | Phase 4 | 구현됨 |

## 1. Health

### `GET /health`

서비스 상태와 버전 정보를 반환합니다.

`/health`는 `/api/v1` 공통 응답 래퍼 대상이 아니며, 현재 구현과 같이 `status: "success"`를 반환합니다.

```json
{
  "status": "success",
  "data": {
    "service": "arproperty-backend",
    "version": "0.1.0"
  }
}
```
...
| 이름 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `lat` | float | O | 위도, 구미 범위 `36.05~36.25` |
| `lon` | float | O | 경도, 구미 범위 `128.20~128.50` |
...
없는 건물 ID는 404 `BUILDING_NOT_FOUND`를 반환합니다.
...
| `year` | int | X | 특정 연도 필터 |
| `page` | int | X | 페이지 번호. 기본값 `1`, 최소 `1` |
| `page_size` | int | X | 페이지 크기. 기본값 `20`, `1~100` 범위로 보정 |
...
없는 건물 ID는 404 `BUILDING_NOT_FOUND`를 반환합니다. 지원하지 않는 `type` 값은 400 `INVALID_PARAMETER`를 반환합니다.
...
| `lat` | float | O | 위도, 구미 범위 `36.05~36.25` |
| `lon` | float | O | 경도, 구미 범위 `128.20~128.50` |
| `radius` | int | X | 반경(m), 기본값 `1000`, 서버에서 `1~3000`으로 보정 |
| `category` | string | X | `medical`, `education`, `convenience`, `transport`, `safety`, `leisure` |
| `page` | int | X | 페이지 번호, 기본값 `1`, 최소 `1` |
| `page_size` | int | X | 페이지 크기, 기본값 `100`, 서버에서 `1~100`으로 보정 |

결과는 거리순으로 정렬되며, `page/page_size` 기준으로 잘라서 반환합니다. Android 지도 호출은 `page/page_size`를 생략해도 기본 첫 페이지를 받을 수 있습니다.
