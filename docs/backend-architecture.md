...
- Redis 컨테이너와 `api_call_log` 테이블은 준비되어 있지만, 현재 서비스 로직은 Redis 캐시나 API 호출 카운트를 사용하지 않는다.
- `CacheService`와 `AppConfig`는 placeholder 수준이다. 캐싱과 호출량 제어는 목표 아키텍처로 남겨두고, 현재 옥계동 AR 태그 시연 경로는 DB/PostGIS 조회를 우선 보호한다.
...
### 무엇을 하는 서버인가?
구미시 아파트의 **위치·시세·생활 편의 정보**를 모아 Android AR 앱에 제공하는 REST API 서버. 사용자가 카메라를 건물에 겨누면 "이 동네는 살기 좋은가? 시세는 얼마인가?"를 즉시 답할 수 있도록, 외부 공공 데이터를 백엔드가 미리 수집·정제하고 단일 API로 노출한다. Redis 캐싱은 인프라만 준비된 다음 단계이다.
...
- **캐시**: Redis 7 컨테이너 구성 완료. 서비스 캐싱 로직은 미연결
...
**하는 일**: 실제 "생각"이 벌어지는 곳. 주문을 받으면 **"이 요리를 어떻게 만들까?"를 결정**한다 — 현재 구현은 DB에서 재료를 꺼내오고(창고), sync 작업에서 외부 API 자료를 수집·정제한다. 캐시 우선 조회는 목표 설계로 남아 있다.

**목표 설계 예시**: `LivabilityService.calculate(buildingId)` — 특정 동(101동)의 생활 편의 점수를 구할 때
```
1) Redis 캐시에 이미 계산된 점수 있나? (CacheService에 물어봄, 현재 미연결)
     → 있으면 바로 반환 (끝)
...
5) 결과를 DB(`building_livability_score`)와 Redis 양쪽에 저장 (목표 설계)
6) 반환
```

**우리 프로젝트의 주요 서비스**: `BuildingService`, `TradeService`, `LivabilityService`, Okgye/VWorld/DataGoKr sync 서비스. `CacheService`는 Redis 단일 창구로 둘 예정이나 현재는 placeholder이다.
...
5) 호출 횟수를 `api_call_log` 테이블에 +1 (목표 설계. 현재 sync 서비스에는 미연결)
```
...
| `building_livability_score` | 건물별 편의 점수 캐시 테이블 | 6개 카테고리 점수 + 종합등급 (S~F), 서비스 계산 연결 예정 |
| `bjdong_code_mapping` | 법정동코드 매핑 | 구미 31개 읍·면·동 코드 시드 |
| `api_call_log` | API 호출 로그 테이블 | 외부 API 일일 호출 한도 추적용 스키마, 서비스 카운트 연결 예정 |
...
Android 클라이언트는 백엔드 한 곳만 알고 있고, 외부 데이터 소스(공공데이터포털·Vworld·Kakao Local)는 모두 백엔드가 단독으로 호출한다. 현재는 사전 적재와 PostGIS 조회가 중심이며, 외부 호출 결과 Redis 캐싱과 `api_call_log.daily_limit` 기반 호출량 제어는 다음 단계 목표이다.
...
flowchart LR
  ...
  API -. "캐시 R/W 예정<br/>Spring Data Redis" .-> REDIS
...
- Redis 인프라와 테이블 스키마는 준비됨 → 실제 서비스 캐싱은 아직 미연결
- 외부 API 호출량은 `api_call_log` 테이블로 일별 카운트·한도 추적 예정

---

## 2. 백엔드 내부 레이어

요청이 들어왔을 때 코드가 어떤 순서로 흐르는지를 5단으로 단순화한 그림. 위에서 아래로 한 방향 흐름이고, `CacheService`는 다음 단계 확장 지점이다.
...
flowchart TB
  ...
  CACHE["CacheService ↔ Redis<br/>(예정)"]
...
| Service | `service/` | 비즈니스 로직, DB 조회, sync 작업 조율 |
| Cache | `service/CacheService` | Redis 읽기/쓰기 단일 창구 예정. 현재 placeholder |
...
**핵심 포인트**
- 현재 `service/`는 DB/PostGIS 조회와 sync 작업을 담당한다
- 향후 `CacheService`가 캐시 관문이 되면 도메인 서비스가 Redis API에 직접 의존하지 않도록 유지한다
...
대표 시나리오: 사용자가 카메라를 건물 쪽으로 겨눠 AR 오버레이가 뜨기까지의 한 사이클. 현재 `GET /api/v1/buildings/nearby`는 Redis를 거치지 않고 PostGIS 반경 검색으로 후보 건물을 조회한다.
...
- Redis 캐시 키 정규화는 향후 동일 좌표·반경 요청을 흡수하기 위한 개선 후보이다
- `/api/v1/buildings/{id}` 상세와 `/api/v1/buildings/{id}/trades` 거래 이력으로 드릴다운한다
...
- `building_livability_score`는 `weight_preset`별 결과 캐싱을 위한 테이블이며, 계산 서비스 연결은 예정
- `api_call_log`는 외부 API 일일 호출 한도(공공데이터 1만, Vworld 4만, Kakao 3만) 통제용 테이블이며, 서비스 카운트 연결은 예정
...
현재 상태: 테이블과 JPA 엔티티/리포지토리는 존재하지만, 생활 편의 점수 계산 결과를 이 테이블이나 Redis에 쓰는 서비스 로직은 아직 연결되지 않았다.
...
현재 상태: 테이블과 시드 데이터는 존재하지만, 외부 API 클라이언트 호출 시 `call_count`를 증가시키거나 `daily_limit`을 차단하는 로직은 아직 연결되지 않았다.
