...
> main 브랜치의 실제 코드/문서 상태를 기반으로 정리되었습니다.
> 기준 일자: 2026-05-08 / 기준 브랜치: `main`
...
- **Backend**: Spring Boot 3.3.5 / Java 17 / Spring Data JPA / Spring Data Redis
- **DB·캐시**: PostgreSQL 16 + PostGIS 3.4 / Redis 7
...
- **구현 완료**
  ...
  - 건물 상세 팝업 UI: 건축물대장·가격 그래프 와이어프레임·거래이력 리스트 (`DetailPopupContent.kt`)
...
- 📸 **필수 캡처 #4**: 건물 상세 팝업(거래이력 리스트 + 가격 그래프 와이어프레임)
...
| `building_livability_score` | 건물별 점수 캐시 (S/A/B/C/D/F + 가중치 프리셋) |
| `bjdong_code_mapping` | 법정동코드 보조 매핑 |
| `api_call_log` | 외부 API 호출 한도 관리 |

- PostGIS 확장 + GIST 공간 인덱스 적용
- 구미시 법정동 24건 초기 데이터 INSERT
- 📸 **필수 캡처 #5**: `docker-compose up -d db redis` 실행 후 `psql` 접속 → `\dt` 결과 (테이블 7개 + 매핑 테이블이 실제 존재함을 증빙)

---

### 슬라이드 8 — Backend 파트 ② API·외부 연동

- **API 엔드포인트 11개** 중 완전 구현은 `/health` 1개 → **약 10%**
- **외부 API 클라이언트 9개** 중 2개 구현 → **약 30%**

| 엔드포인트 | 상태 |
|---|---|
| `GET /health` | ✅ 완전 구현 (DB·Redis 헬스 포함) |
| `GET /api/v1/buildings/nearby` | 📋 스켈레톤 |
| `GET /api/v1/buildings/{id}` | 📋 스켈레톤 |
| `GET /api/v1/buildings/{id}/trades` | 📋 스켈레톤 |
| `GET /api/v1/complexes` / `/{id}` / `/{id}/buildings` / `/{id}/trades` | 📋 스켈레톤 |
| `GET /api/v1/livability/{id}` / `/compare` | 📋 스켈레톤 (Phase 4) |
| `GET /api/v1/infra/nearby` | 📋 스켈레톤 (Phase 4) |
...
- **백엔드 데모(선택)**: `/health` 응답으로 DB/Redis 살아있음 시연
...
- **공통 차단 이슈**
  - ViewModel ↔ Repository 미연결 → Android에서 어떤 API도 실제로 호출하지 않음
  - Backend Controller·Service 로직 미작성 → 호출 가능한 엔드포인트는 `/health` 뿐
  - Data 수집 스크립트 본문 미구현 → DB에 들어갈 실데이터 없음
- **기능 미구현**
  - AR ↔ 지도 상태 공유 (선택한 건물을 두 화면에서 동기)
  - 지도 건물 마커 / 편의시설 필터칩 동작
  - 건물 상세·단지 상세·생활점수 화면 데이터 표시
  - 가격 변동 실데이터 그래프 (현재 와이어프레임만)
- **리스크**
...
| 4 | 건물 상세 화면에 `DetailPopupContent` 데이터 바인딩 | R4 | 거래이력·가격 그래프 라이브화 |
| 5 | 통합 QA 체크리스트 작성·실행 | R1 | 다음 발표 시 정량 진척도 측정 가능 |
...
| 상세화면 UI 골격 | 30% | `BuildingDetailRoute.kt`, `ComplexDetailRoute.kt`, `LivabilityRoute.kt` |
| Retrofit·Repository 정의 | 80% | `ApiServices.kt:15-72`, `AppContainer.kt:1-140` |
| ViewModel ↔ Repository 연결 | 0% | 모든 Route의 ViewModel이 정적 상태만 보유 |
